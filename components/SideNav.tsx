  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-bold text-base tracking-tight" style={{ color: 'var(--color-text-1)' }}>
          OlyRetail
        </span>
      </div>

      {/* Nav — always expanded, no dropdowns */}
      <nav className="flex-1 py-4 px-3 space-y-5 overflow-y-auto">
        {NAV_SECTIONS.map((section) => (
          <div key={section.label}>
            ...
            >
              <span style={{ color: 'var(--color-text-4)' }}>{section.icon}</span>
              <span
                className="text-xs font-semibold uppercase tracking-widest"
                style={{ color: 'var(--color-text-4)', letterSpacing: '0.07em' }}
              >
                {section.label}
              </span>
            </div>
            ...
            <div className="space-y-0.5">
              {section.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <span style={{ flexShrink: 0, color: active ? 'var(--color-primary)' : 'var(--color-text-4)' }}>
                      {item.icon}
                    </span>
