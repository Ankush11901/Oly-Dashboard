...
function StatCard({ label, value, badge, icon, iconColor }: { label: string; value: string; badge: string; icon: React.ReactNode; iconColor: string }) {
  return (
    <div className="card">
      ...
      <p className="font-bold tabular-nums" style={{ fontSize: 26, color: '#111827', lineHeight: 1.2 }}>{value}</p>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 560, maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.18)', overflow: 'hidden' }}
      >
        {/* Header */}
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: '#EEE9FF', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <ShieldCheck size={18} strokeWidth={1.5} style={{ color: '#655BD3' }} />
          </div>
          <div>
            <p style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>New QS Profile</p>
            <p style={{ fontSize: 12, color: '#9CA3AF' }}>Define qualification criteria for this profile</p>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              <label style={{ display: 'block', fontSize: 11.5, fontWeight: 700, color: '#374151', marginBottom: 6, letterSpacing: '0.04em', textTransform: 'uppercase' }}>Profile Name</label>
              <input
                autoFocus
                type="text"
            ...
            <div>
              <label style={{ display: 'block', fontSize: 11.5, fontWeight: 700, color: '#374151', marginBottom: 6, letterSpacing: '0.04em', textTransform: 'uppercase' }}>Description <span style={{ color: '#9CA3AF', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional)</span></label>
              <input
                type="text"
          ...
          {/* Criteria table */}
          <div>
            <label style={{ display: 'block', fontSize: 11.5, fontWeight: 700, color: '#374151', marginBottom: 10, letterSpacing: '0.04em', textTransform: 'uppercase' }}>Criteria</label>
            <div style={{ border: '1px solid #E5E7EB', borderRadius: 10, overflow: 'hidden' }}>
              {criteria.map((c, i) => (
                <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderTop: i === 0 ? 'none' : '1px solid #F3F4F6' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: '#111827' }}>{c.label}</p>
                  </div>
                  <input
                    type="number"
                    min={1}
                    ...
                    style={{ width: 60, border: '1px solid #E5E7EB', borderRadius: 6, padding: '5px 8px', fontSize: 13, fontWeight: 700, color: '#655BD3', textAlign: 'right', outline: 'none', background: '#F9FAFB' }}
                    onFocus={e => (e.currentTarget.style.borderColor = '#655BD3')}
                    onBlur={e => (e.currentTarget.style.borderColor = '#E5E7EB')}
                  />
                  <span style={{ fontSize: 11.5, color: '#9CA3AF', width: 72 }}>{c.unit}</span>
                  <button
                    onClick={() => removeRow(c.id)}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#D1D5DB', padding: 2, display: 'flex', borderRadius: 4 }}
              ...
              {/* Inline add row */}
              {addingRow && (
                <div style={{ padding: '10px 14px', borderTop: '1px solid #F3F4F6', background: '#F9FAFB', display: 'flex', gap: 8, alignItems: 'center' }}>
                  <input
                    autoFocus
                    type="text"
                    ...
                    style={{ flex: 1, border: '1px solid #DDD6FE', borderRadius: 6, padding: '5px 8px', fontSize: 13, color: '#111827', outline: 'none', background: 'white' }}
                  />
                  <input
                    type="number"
                    min={1}
                    ...
                    style={{ width: 60, border: '1px solid #DDD6FE', borderRadius: 6, padding: '5px 8px', fontSize: 13, fontWeight: 700, color: '#655BD3', textAlign: 'right', outline: 'none', background: 'white' }}
                  />
                  <input
                    type="text"
                    placeholder="unit"
                    ...
                    style={{ width: 68, border: '1px solid #DDD6FE', borderRadius: 6, padding: '5px 8px', fontSize: 12, color: '#6B7280', outline: 'none', background: 'white' }}
                  />
                  <button onClick={addRow} style={{ background: '#655BD3', border: 'none', borderRadius: 6, color: 'white', padding: '5px 8px', cursor: 'pointer', display: 'flex' }}>
                    <Check size={13} strokeWidth={2.5} />
                  </button>
                  <button onClick={() => setAddingRow(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9CA3AF', padding: 2, display: 'flex' }}>
                    <X size={13} strokeWidth={2.5} />
                  </button>
                </div>
              )}
            ...
            {!addingRow && (
              <button
                ...
                style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 6, fontSize: 12.5, fontWeight: 500, color: '#655BD3', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
              >
        ...
        <div style={{ padding: '14px 24px', borderTop: '1px solid #F3F4F6', display: 'flex', gap: 10, flexShrink: 0 }}>
          <button
            onClick={onClose}
            style={{ flex: 1, height: 38, borderRadius: 8, border: '1px solid #E5E7EB', background: 'white', fontSize: 13, fontWeight: 500, color: '#374151', cursor: 'pointer' }}
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={!name.trim()}
            style={{ flex: 2, height: 38, borderRadius: 8, border: 'none', background: name.trim() ? '#655BD3' : '#E5E7EB', fontSize: 13, fontWeight: 600, color: name.trim() ? 'white' : '#9CA3AF', cursor: name.trim() ? 'pointer' : 'not-allowed', transition: 'background 150ms ease' }}
          >
            Save Profile
          </button>
        </div>
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: '#111827' }}>Qualified Shoppers</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <StatCard label="Conversion Rate"  value="34.2%"  badge="+3.1%"  icon={<TrendingUp size={20} strokeWidth={1.5} />} iconColor="#655BD3" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <p style={{ fontSize: 11.5, fontWeight: 700, color: '#9CA3AF', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 4 }}>
          ...
          {profiles.map(p => {
            ...
            return (
              <div
                ...
                onMouseEnter={e => { if (!active) (e.currentTarget as HTMLElement).style.borderColor = '#DDD6FE'; }}
                onMouseLeave={e => { if (!active) (e.currentTarget as HTMLElement).style.borderColor = '#E5E7EB'; }}
              >
                {/* Active indicator */}
                {active && <div style={{ position: 'absolute', left: 0, top: '20%', bottom: '20%', width: 3, borderRadius: '0 3px 3px 0', background: '#655BD3' }} />}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                      ...
                      <div style={{
                        ...
                        border: `2px solid ${active ? '#655BD3' : '#D1D5DB'}`,
                        background: active ? '#655BD3' : 'white',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                      }}>
                        {active && <div style={{ width: 5, height: 5, borderRadius: '50%', background: 'white' }} />}
                      </div>
                      <p style={{ fontSize: 13, fontWeight: 700, color: active ? '#655BD3' : '#111827', truncate: true } as React.CSSProperties}>{p.name}</p>
                      {p.isDefault && (
                        <span style={{ fontSize: 9.5, fontWeight: 700, padding: '1px 6px', borderRadius: 4, background: '#EEE9FF', color: '#655BD3', letterSpacing: '0.04em', flexShrink: 0 }}>DEFAULT</span>
                      )}
                    </div>
                    {p.description && (
                      <p style={{ fontSize: 11.5, color: '#9CA3AF', marginLeft: 20, lineHeight: 1.45 }}>{p.description}</p>
                    )}
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginLeft: 20, marginTop: 8 }}>
                      {p.criteria.map(c => (
                        <span key={c.id} style={{ fontSize: 10.5, fontWeight: 600, padding: '2px 7px', borderRadius: 4, background: active ? '#EEE9FF' : '#F3F4F6', color: active ? '#655BD3' : '#6B7280' }}>
                          {c.value} {c.unit}
                        </span>
                      ))}
                    </div>
          ...
          <button
            ...
            style={{
              ...
              border: '1.5px dashed #DDD6FE', background: 'transparent',
              fontSize: 12.5, fontWeight: 600, color: '#655BD3', cursor: 'pointer',
              transition: 'all 150ms ease',
            }}
        ...
        {activeProfile && (
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #F3F4F6', display: 'flex', alignItems: 'center', gap: 10 }}>
              <ShieldCheck size={16} strokeWidth={1.5} style={{ color: '#655BD3' }} />
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: '#111827' }}>{activeProfile.name}</p>
                {activeProfile.description && (
                  <p style={{ fontSize: 11.5, color: '#9CA3AF', marginTop: 1 }}>{activeProfile.description}</p>
                )}
              </div>
              <ChevronRight size={15} strokeWidth={2} style={{ color: '#9CA3AF' }} />
            ...
            {activeProfile.criteria.map((c, idx) => (
              <div
                key={c.id}
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', borderTop: idx === 0 ? 'none' : '1px solid #F3F4F6' }}
              >
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 13.5, fontWeight: 600, color: '#111827' }}>{c.label}</p>
                  <p style={{ fontSize: 11.5, color: '#9CA3AF', marginTop: 2 }}>{c.description}</p>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <input
                    ...
                    style={{
                      width: 68, border: '1px solid #E5E7EB', borderRadius: 8,
                      padding: '7px 10px', fontSize: 15, fontWeight: 700,
                      color: '#655BD3', textAlign: 'right', outline: 'none', background: '#F9FAFB',
                    }}
                    onFocus={e => (e.currentTarget.style.borderColor = '#655BD3')}
                    onBlur={e => (e.currentTarget.style.borderColor = '#E5E7EB')}
                  />
                  <span style={{ fontSize: 12, color: '#9CA3AF', minWidth: 72 }}>{c.unit}</span>
                </div>
              </div>
            ))}
      ...
      <div className="card">
        <p className="text-sm font-semibold mb-4" style={{ color: '#111827' }}>Top Qualified Shoppers Today</p>
        <div style={{ maxHeight: 320, overflowY: 'auto' }}>
          {SHOPPERS.map((s, idx) => (
            <div
              key={s.name}
              className="flex items-center gap-4 py-3"
              style={{ borderBottom: idx < SHOPPERS.length - 1 ? '1px solid #F3F4F6' : 'none' }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = '#F9FAFB'}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}
            >
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ background: 'rgba(101,91,211,0.15)', color: '#655BD3' }}>
                {s.initials}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate" style={{ color: '#111827' }}>{s.name}</p>
                <p className="text-xs truncate mt-0.5" style={{ color: '#9CA3AF' }}>{s.store}</p>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full" style={{ background: 'rgba(101,91,211,0.12)', color: '#655BD3' }}>{s.zones} zones</span>
                <span className="text-xs tabular-nums" style={{ color: '#9CA3AF', minWidth: 48, textAlign: 'right' }}>{s.visits} visits</span>
              </div>
            </div>
          ))}
        </div>
